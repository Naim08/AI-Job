...
import { UserProfile, JobListing } from "../src/shared/types.ts";
import { ensureSession, isCheckpoint } from "./session.ts";
import { scoreJob } from "./filter.ts";
...
export async function scanLinkedInJobs(user: UserProfile): Promise<void> {
  log(`Starting LinkedIn job scan for user ${user.user_id}`);
  ...
  log(`Fetching profile settings for user ${user.user_id}`);
        ...
        try {
          ...
          for (const card of jobCards) {
            ...
            try {
              ...
              log(
                `[Scanner] Attempting to upsert job application with user_id: ${user.user_id}, job_id: ${jobId}`
              );
