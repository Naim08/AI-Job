...
export async function scanLinkedInJobs(user: UserProfile): Promise<void> {
  ...
  if (profileError) {
    ...
    await context.close();
    throw new Error(`Failed to fetch profile settings: ${profileError.message}`);
  }
  ...
  const keywords = (userKeywords && userKeywords.length > 0) ? userKeywords : ['Software Engineer'];
  const locations = (userLocations && userLocations.length > 0) ? userLocations : ['Remote'];
    ...
    for (const keyword of keywords) {
      for (const location of locations) {
        ...
        try {
          ...
          const jobCards = await page.locator('li.scaffold-layout__list-item').all();
          log(`Found ${jobCards.length} job cards`);
          
          let processedInThisSearch = 0;
          for (const card of jobCards) {
            if (totalProcessed >= MAX_TOTAL_JOBS || processedInThisSearch >= 25) {
              break;
            }
            const cardDebugId = await card.getAttribute('data-occludable-job-id') || `index-${processedInThisSearch}`;
            log(`[Card ${cardDebugId}] Starting processing.`);
            
            try {
              log(`[Card ${cardDebugId}] Checking for Easy Apply...`);
              const isEasyApply = await card.getByText('Easy Apply').isVisible().catch(() => false);
              ...
              if (!jobId) {
                log(`[Card ${cardDebugId}] Skipping - could not find data-occludable-job-id on card`);
                continue;
              }
              
              log(`[Card ${cardDebugId}] Extracting title...`);
              const titleLinkElement = card.locator('a.job-card-list__title--link').first();
              let title = await titleLinkElement.getAttribute('aria-label', { timeout: 5000 });
              if (!title || title.trim() === '') {
                log(`[Card ${cardDebugId}] aria-label not found for title, trying textContent...`);
              ...
              const companySubtitleElement = card.locator('div.artdeco-entity-lockup__subtitle').first();
              const company = await companySubtitleElement.textContent({ timeout: 10000 }).catch(() => 'Unknown Company') || 'Unknown Company';
              log(`[Card ${cardDebugId}] Company: ${company.substring(0, 50)}...`);
              
              // log(`[Card ${cardDebugId}] Pausing to inspect card for location. Type playwright.resume() in browser console to continue.`);
              // await page.pause();

              log(`[Card ${cardDebugId}] Extracting location text from card...`);
              let locationText = '';
              try {
                const metadataListItems = await card.locator('.job-card-container__metadata-wrapper > li').all();

                if (metadataListItems.length > 0) {
                    log(`[Card ${cardDebugId}] Found ${metadataListItems.length} item(s) in card\'s metadata list using '.job-card-container__metadata-wrapper > li'.`);
                    const texts = [];
                    ...
                    if (locationText) {
                        log(`[Card ${cardDebugId}] Location from '.job-card-container__metadata-wrapper > li': ${locationText.substring(0,50)}...`);
                    } else {
                        log(`[Card ${cardDebugId}] '.job-card-container__metadata-wrapper > li' yielded no text.`);
                    }
                } else {
                    log(`[Card ${cardDebugId}] No list items found with selector '.job-card-container__metadata-wrapper > li'. Trying fallback: 'span.job-card-container__metadata-item'.`);
                }

                // Fallback if primary selector failed or yielded no text
                if (!locationText) {
                    const metadataSpans = await card.locator('span.job-card-container__metadata-item').all();
                    if (metadataSpans.length > 0) {
                        log(`[Card ${cardDebugId}] Found ${metadataSpans.length} span(s) with class 'job-card-container__metadata-item'.`);
                        for (const span of metadataSpans) {
                            const spanText = await span.textContent({ timeout: 2000 });
                            const trimmedText = spanText?.trim();
                            if (trimmedText && !trimmedText.toLowerCase().includes('easy apply') && trimmedText.length > 1) {
                                locationText = trimmedText;
                                log(`[Card ${cardDebugId}] Location from 'span.job-card-container__metadata-item': ${locationText.substring(0,50)}...`);
                                break; // Use the first valid one
                            }
                        }
                        if (!locationText) {
                            log(`[Card ${cardDebugId}] 'span.job-card-container__metadata-item' yielded no suitable location text after filtering.`);
                        }
                    } else {
                        log(`[Card ${cardDebugId}] No spans found with selector 'span.job-card-container__metadata-item'.`);
                    }
                }
              } catch (e: any) {
                log(`[Card ${cardDebugId}] Error extracting location from card: ${e.message}`);
              }

              if (!locationText) {
                log(`[Card ${cardDebugId}] Location could not be determined. Defaulting to 'Unknown Location'.`);
                locationText = 'Unknown Location';
              }
              
              log(`[Card ${cardDebugId}] Final Location text for card: ${locationText.substring(0, 50)}...`);
              
              log(`[Card ${cardDebugId}] Locating clickable card area...`);
              const clickableCardArea = card.locator('div.job-card-container--clickable');
              log(`[Card ${cardDebugId}] Checking if clickable card area is visible...`);
              const isClickableAreaVisible = await clickableCardArea.isVisible({timeout: 5000});
              log(`[Card ${cardDebugId}] Clickable area visible: ${isClickableAreaVisible}`);
              
              if (isClickableAreaVisible) {
                log(`[Card ${cardDebugId}] Clicking specific clickable card area...`);
              ...
              log(`[Card ${cardDebugId}] Waiting for job description content...`);
              await page.waitForSelector('div.jobs-description-content__text--stretch', { timeout: 10000 });
              log(`[Card ${cardDebugId}] Job description content found.`);
              
              const description = await page.locator('div.jobs-description-content__text--stretch').textContent({ timeout: 10000 }) || '';
              log(`[Card ${cardDebugId}] Description length: ${description.length}`);
              ...
              log(`Evaluating job: ${jobListing.title} at ${jobListing.company} (ID: ${jobId})`);
              ...
              if (score.blacklisted || score.similarity < 0.65) {
                ...
                if (score.blacklisted) {
                  ...
                  reason = `Low similarity score: ${score.similarity.toFixed(2)}`;
                }
              }
          ...
          log(`Processed ${processedInThisSearch} jobs for '${keyword}' in '${location}'`);
        } catch (error: any) {
          log(`Error searching for '${keyword}' in '${location}': ${error.message}`);
        } finally {
...
if (import.meta.url.startsWith('file:') && process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  const userIdArg = args.find(arg => arg.startsWith('--user=') || arg === '--user');
  ...
  if (!userId) {
    ...
    console.error('Usage: npx node --loader ts-node/esm agent/scanner.ts --user <USER_ID>');
    process.exit(1);
  }
