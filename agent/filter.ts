...
async function fetchSimilarResumeChunks(userId: string, jobEmbedding: number[], limit: number): Promise<ResumeChunk[]> {
  log(`Fetching top ${limit} resume chunks for user ${userId} based on job embedding`);
...
function cosineSimilarity(vecA: ReadonlyArray<number>, vecB: ReadonlyArray<number>): number {
...
export async function scoreJob(user: UserProfile, job: JobListing): Promise<{ score: FilterScore; trace: DecisionNode }> {
  log(`Scoring job "${job.title}" from "${job.company}" for user "${user.name}"`);

  // 1. Blacklist gate
  const blacklistItems = await fetchBlacklistItems(user.id);
  const companyLower = job.company.toLowerCase();
  const isBlacklisted = blacklistItems.some(
    item => item.type === 'company' && companyLower.includes(item.value.toLowerCase())
  );
  log(isBlacklisted ? `Company "${job.company}" is blacklisted.` : `Company "${job.company}" is not blacklisted.`);
  ...
  if (!isBlacklisted) {
    ...
    topResumeChunks = await fetchSimilarResumeChunks(user.id, jobEmbedding as number[], 10);

    if (topResumeChunks.length > 0) {
      similarity = Math.max(
        ...topResumeChunks.map(chunk => cosineSimilarity(jobEmbedding, chunk.embedding))
      );
  ...
  const filterScore: FilterScore = {
    ...
    explanation: `Company Blacklisted: ${isBlacklisted}. Max Similarity: ${similarity.toFixed(4)}.`,
