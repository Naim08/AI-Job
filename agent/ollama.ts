...
export async function listModels(): Promise<OllamaModelInfo[]> {
  if (process.env.SKIP_OLLAMA_BOOTSTRAP === '1') {
    debug('ollama', 'SKIP_OLLAMA_BOOTSTRAP set, returning not installed for all');
    return SUPPORTED_MODELS.map(name => ({ name, installed: false, sizeMB: 0 }));
  }
  try {
    ...
    for (const line of lines) {
      ...
      const name = cols[0];
      const sizeStr = cols[2] && cols[2].toLowerCase().match(/(\d+\.?\d*)\s*(gb|mb)/) ? cols[2] : cols[1];
      let sizeMB = 0;
