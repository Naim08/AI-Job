...
import type {
  ...
  JobListing,
  ApplicationStatus,
} from "../src/shared/types.js"; // Path as corrected by user
...
async function manualTest() {
  ...
  const sampleJob: JobListing = {
    ...
    company: "Innovatech",
    description:
      "Seeking an expert in integrating LLMs into existing software products. Experience with RAG pipelines and local model deployment (Ollama) is a plus. We value proactive problem solvers.",
  ...
  try {
    ...
    answers.forEach((ans, index) => {
      console.log(
        `Validating Answer ${index + 1} for question: "${ans.question}"`
      );
      let answerChecksPass = true;
      if (!(ans.answer.length > 20)) {
        console.error(
          `  [FAIL] Answer ${index + 1} length is not > 20. Length: ${
            ans.answer.length
          }`
        );
        ...
        console.log(
          `  [PASS] Answer ${index + 1} length is > 20. Length: ${
            ans.answer.length
          }`
        );
      }
      ...
      if (!(ans.refs.length >= 1)) {
        console.error(
          `  [FAIL] Answer ${index + 1} refs is not >= 1. Refs: ${
            ans.refs.join(", ") || "None"
          }`
        );
        answerChecksPass = false;
        ...
        console.log(
          `  [PASS] Answer ${index + 1} refs is >= 1. Refs: ${ans.refs.join(
            ", "
          )}`
        );
      }
      ...
      if (ans.answer.length > 400) {
        console.error(
          `  [FAIL] Answer ${index + 1} length EXCEEDS 400. Length: ${
            ans.answer.length
          }`
        );
        answerChecksPass = false;
        ...
        console.log(
          `  [PASS] Answer ${index + 1} length is <= 400. Length: ${
            ans.answer.length
          }`
        );
      }
      if (!answerChecksPass)
        console.log(`  Full Answer ${index + 1}: ${ans.answer}`);
    });
    ...
    if (!(coverLetter.length > 20)) {
      console.error(
        `  [FAIL] Cover letter length is not > 20. Length: ${coverLetter.length}`
      );
      coverLetterPass = false;
    } else {
      console.log(
        `  [PASS] Cover letter length is > 20. Length: ${coverLetter.length}`
      );
    }

    if (!(coverLetter.length <= 2000)) {
      console.error(
        `  [FAIL] Cover letter length EXCEEDS 2000. Length: ${coverLetter.length}`
      );
      coverLetterPass = false;
    } else {
      console.log(
        `  [PASS] Cover letter length is <= 2000. Length: ${coverLetter.length}`
      );
    }
